...
$sql="update employeeleave set F_name=:fname, L_name=:lname, Gender=:gender, dateob=:dob, Department=:department, Address=:address, City=:city, Country=:country, Phonenumber=:mobileno where EmailId=:eid";
...
<html lang="en">
    <head>
        ...
        <meta name="author" content="Abdul Hafiz" />
...
<?php 
$eid=$_SESSION['emplogin'];
$sql = "SELECT * from  employeeleave where EmailId=:eid";
$query = $dbh -> prepare($sql);
$query -> bindParam(':eid',$eid, PDO::PARAM_STR);
$query->execute();
...
<input id="firstName" name="firstName" value="<?php echo htmlentities($result->F_name);?>"  type="text" required>
</div>

<div class="input-field col m6 s12">
<label for="lastName">Last name </label>
<input id="lastName" name="lastName" value="<?php echo htmlentities($result->L_name);?>" type="text" autocomplete="off" required>
</div>
...
<input id="birthdate" name="dob"  class="datepicker" value="<?php echo htmlentities($result->dateob);?>" >
</div>
...
<?php $sql = "SELECT DepartmentName from leavedept";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
